...
const SOCIALS = [
  { href: "https://www.youtube.com/@jamon_hd", label: "YouTube", icon: "youtube" },
  { href: "https://www.instagram.com/jamononinsta/", label: "Instagram", icon: "instagram" },
  { href: "https://www.tiktok.com/@jamon_tiktok", label: "TikTok", icon: "tiktok" },
...
function socialLinksHTML() {
  return SOCIALS.map(
    (s) => `<a href="${s.href}" target="_blank" rel="noopener" aria-label="${s.label}" data-track-click="social-${s.icon}"><svg viewBox="0 0 24 24" fill="currentColor">${ICONS[s.icon]}</svg></a>`
  ).join("");
}
  ...
  <header class="site-header">
    <div class="container">
      <a class="site-logo" href="/">
        <img src="https://zevkev.github.io/Medienspeicher/Bilder/Jamon.jpg" alt="">
        Jamon
      </a>
...
function footerHTML() {
  ...
  <footer class="site-footer">
    <div class="footer-inner">
      ...
      <p class="footer-note">&copy; ${new Date().getFullYear()} Jamon. Freizeitpark-Content und Gaming.</p>
    </div>
  </footer>`;
}

// Owner-only "Privat" nav/footer link to /privat/ -- shown site-wide for
// EITHER Medienspeicher owner (Kevin or Jason -- isOwner() checks the
// shared OWNER_EMAILS list in js/firebase-config.js, same Firebase project
// as zevkev.de), not just this site's own host, since moderation/campaigns
// here are the same shared account system. Reactive via onAuthChange so it
// appears/disappears immediately on sign-in/out without a reload.
...
export async function mountLayout() {
  ...
  // watchlist, resume position) -- YouTube, Watchlist and the individual
  // video pages. Everywhere else (home, shop, legal pages) it'd just be a
  // button that does nothing relevant, so it's left out of the header
  // entirely there rather than shown and unused.
  const showAccount =
    path.startsWith("/youtube") ||
    path.startsWith("/watchlist") ||
    path.startsWith("/watch") || // covers both /watch/?id=... and the clean /video/<id>/ form
  ...
  // -- shown on every video page regardless of whether the watchlist is
  // empty yet, hidden everywhere else (home/shop/legal).
  if (!showAccount) {
