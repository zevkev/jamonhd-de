...
// Icon paths for youtube/instagram/tiktok/discord are copied verbatim from
// js/layout.js's own ICONS (the already-correct, already-verified versions).
// twitch isn't in this site's layout.js ICONS (Jamon doesn't stream) so it's
// copied from the identical, already-verified icon used on zevkev.de's own
// copy of this file instead. x/website are new, deliberately built from
// simple primitives (straight lines / a circle) rather than a hand-recalled
// logo silhouette, to avoid risking a broken-icon bug.
...
// (see js/user-theme.js). "default" isn't in this map on purpose -- profil.js
// falls back to the page's own normal mat when a profile has no background
// set (or an unrecognized/legacy key), so removing a preset later can't
// strand existing profiles on a blank page. Solid colors only, deliberately
// -- applied by overriding the --blue-mat custom property that body's own
// background-color already uses (css/style.css keeps that name across both
// sites on purpose -- see its own header comment -- even though it's green
// here, not blue), so every grid/texture layer already painted on top of it
// keeps working unmodified. A gradient would silently fail there
// (background-color never accepts one).
