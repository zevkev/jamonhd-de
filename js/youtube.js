...
function renderFeatured(video) {
  ...
  if (!video) {
    ...
    heroContentEl.innerHTML = `
      <div class="yt-hero-empty">
        ...
        <a class="yt-empty-cta" href="https://www.youtube.com/@jamon_hd" target="_blank" rel="noopener">Zum YouTube-Kanal</a>
      </div>`;
    return;
  }
  ...
  heroContentEl.innerHTML = `
    ...
    <div class="yt-hero-actions">
      ...
      <a class="btn-youtube" href="https://www.youtube.com/@jamon_hd?sub_confirmation=1" target="_blank" rel="noopener" data-track-click="youtube-subscribe">${youtubeGlyph()}Abonnieren</a>
...
function renderGrid(videos, mode) {
  ...
  if (!list.length) {
    const emptyHTML = query
      ...
      : `<div class="empty-state">${emptyIcon()}<h2>${mode === "shorts" ? "Noch keine Shorts" : "Noch keine Videos"}</h2><p>Schau bald wieder vorbei.</p><a class="yt-empty-cta" href="https://www.youtube.com/@jamon_hd" target="_blank" rel="noopener">Zum YouTube-Kanal</a></div>`;
  ...
  // only ever looked fine because css/youtube.css's .vod-card ALSO carried
  // its own immediate (non-scroll-triggered) fade-in animation, independent
  // of this class entirely, which faded every card in right after render
  // regardless of scroll position -- the actual cause of "scrolling
  // doesn't do anything" on this page. That redundant animation is gone
  // now; this is the one real mechanism.
  initReveal(gridEl);
