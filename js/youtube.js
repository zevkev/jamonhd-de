...
function renderGrid(videos, mode) {
  ...
  if (!list.length) {
    gridEl.innerHTML = `<div class="empty-state">${emptyIcon()}<h2>${mode === "shorts" ? "Noch keine Shorts" : "Noch keine Videos"}</h2><p>Schau bald wieder vorbei.</p><a class="yt-empty-cta" href="https://www.youtube.com/@jamon_hd" target="_blank" rel="noopener">Zum YouTube-Kanal</a></div>`;
