...
export async function updateProfileCustomization(bio, socials, background) {
  ...
  for (const [key, value] of Object.entries(socials || {})) {
    if (!SOCIAL_PLATFORMS[key]) continue;
...
// watchlist.js) at once instead of each page having to remember to gate its
// own init() on onAuthChange the way account.js already did.
...
export async function deleteAccount() {
  ...
  // null for a legitimately signed-in user (confirmed live on zevkev.de: an
  // interrupted signup left exactly this state, see login.js's onAuthChange
  // race fix), and a stale/empty displayName here used to silently skip
  // releasing the real reservation, permanently blocking that name for
  // anyone else even after this account was gone. Querying by uid finds it
  // regardless of whatever displayName says.
