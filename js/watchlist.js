// Watchlist page: reads the same watchlist store js/youtube.js writes to
// (js/user-data.js -- localStorage when logged out, synced through the
// visitor's account when logged in), then resolves each saved id against
// the one video feed this site has (main-videos.json).
...
// Same small icon set convention as youtube.js: each page script owns its
// own copies rather than importing a shared module.
...
// Populated once in init(), keyed by video id -> the video object plus an
// isShort-derived `source` tag. Re-read on every render so a re-render
// always reflects the current localStorage state (e.g. right after an
// un-star click).
let byId = new Map();

function sourceBadgeHTML(source) {
  if (source === "short") return `<span class="wl-source-badge wl-source-badge--short">${boltIcon()}Short</span>`;
  return `<span class="wl-source-badge wl-source-badge--youtube">YouTube</span>`;
}
...
function cardHTML(video) {
  ...
  return `
  <a href="/video/${video.id}/" class="vod-card rip reveal" data-video-id="${video.id}">
    <div class="vod-thumb">
...
function emptyStateHTML() {
  ...
  <div class="empty-state">
    ...
    <p>Speicher Videos mit dem Stern auf der <a href="/youtube/">YouTube</a> Seite, um sie hier wiederzufinden.</p>
  </div>`;
}
...
function renderGrid() {
  ...
  for (const v of list) {
    ...
    (progressCache.get(key)?.watched ? watched : unwatched).push(v);
  }
...
async function init() {
  ...
  try {
    const [mainFeed] = await Promise.all([loadJSON("/assets/data/main-videos.json", { videos: [] })]);
