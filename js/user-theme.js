...
// Same key name as zevkev-de's own copy of this file on purpose (see
// js/theme.js's identical "zevkev-theme" key here too) -- localStorage is
// already origin-scoped, so there's no actual collision risk between the
// two sites, and matching names keeps the two codebases' shared conventions
// simple to keep in sync.
const CACHE_KEY = "zevkev-user-bg-cache";

// The homepage is a special case: body.home doesn't use --blue-mat at all
// -- css/home.css gives its own hero/discord sections two separately-tuned
// shades of their own (--zone-hero/--zone-discord) for visual rhythm
// between sections, deliberately bypassing the shared mat token (see that
// file's own comment on why). Confirmed live on zevkev-de (same pattern,
// three zone tokens there since that site also has a Twitch zone): only
// overriding --blue-mat left the homepage showing its default color while
// every other page correctly recolored. This site's own homepage only has
// two zone tokens (no Twitch content, no --zone-twitch) -- flattening both
// to the same chosen color means the homepage loses that section-to-section
// shade variation in exchange for actually being personalized like the
// user asked (color cascades "everywhere, not just the profile tab") --
// every other page only has the one --blue-mat token to begin with, so
// this is homepage-only.
const ZONE_VARS = ["--zone-hero", "--zone-discord"];
