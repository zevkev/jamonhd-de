...
//
// site == "jamon": campaigns is a SHARED collection with zevkev-de (same
// Firebase project) -- without this, a ZevKev-only promo code would show
// here too (different Fourthwall store, code likely invalid on this one).
async function loadCampaignBanner() {
  const el = document.getElementById("shop-campaign-banner");
  if (!el) return;
  try {
    const snap = await getDocs(query(collection(db, "campaigns"), where("published", "==", true), where("site", "==", "jamon")));
